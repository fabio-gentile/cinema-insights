...
const submitFilter = () => {
  ...
  filteredMovies.value = filteredMovies.value.filter((movie) => {
    const movieReleaseDate = new Date(movie.first_air_date)
...
<template>
  <TitlePart :title="'Popular TV Shows'" class="mt-8" />
  <div class="flex gap-8 flex-col lg:flex-row">
    ...
    >
      ...
      <div class="p-4 grid gap-8">
        ...
        <div v-if="!isLoadingTvShowGenres && tvShowGenres" class="grid gap-4">
          <h4 class="font-semibold text-lg">Genres</h4>
          <ul class="flex gap-4 flex-wrap">
            <li
              @click="addFilter(genre.id)"
              v-for="genre in tvShowGenres.genres"
              :key="genre.id"
              :class="[
    ...
    <!--      movie-->
    <div v-if="isLoadingPopularTvShows">Loading...</div>
    <div class="w-full" v-else>
      <div v-if="filteredMovies.length === 0">No tv shows found. Try changing your filters</div>
        ...
        >
          <RouterLink
            :to="{
              ...
              params: {
                id: movie.id,
                title: (movie.title !== undefined ? movie.title : movie.name).replaceAll(' ', '-').toLowerCase()
              }
            }"
      ...
      <button
        @click="() => fetchNextPagePopularTvShows()"
        :disabled="!hasNextPagePopularTvShows || isFetchingNextPagePopularTvShows"
